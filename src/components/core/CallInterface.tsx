...
export default function CallInterface() {
    ...
    const makeCall = async (number: string, isAppToApp: boolean = false) => {
        ...
        console.log('device.state:', device?.state);
        console.log('number:', number);
        console.log('isAppToApp:', isAppToApp);
        ...
        try {
            ...
            // Use custom parameter names to avoid conflicts with Twilio's reserved parameters
            const params: any = {
                customTo: number,  // Use customTo instead of To
                isAppToApp: isAppToApp ? 'true' : 'false',
                callType: isAppToApp ? 'app-to-app' : 'app-to-phone',
                timestamp: Date.now().toString()  // Add timestamp for debugging
            };

            console.log('Call parameters being sent:', params);
            const call = await device.connect(params);
            console.log('Call object created:', call);
            setActiveCall(call);
